...
def CV_data(Valve_Type, D_in, Valve_position):
    ...
    if Valve_Type not in _VALVE_DATA:
        raise ValueError(f"Valve_Type {Valve_Type} not supported (1=Concentric, 2=Triple Offset)")

    data = _VALVE_DATA[Valve_Type]
    D_vals = data['D']
    Pos_vals = data['Pos']
    CV_vals = data['CV']

    d_eval = float(np.clip(D, D_vals[0], D_vals[-1]))
    pos_eval = float(np.clip(Valve_position, Pos_vals[0], Pos_vals[-1]))
    interp = _cv_interpolator(Valve_Type)
    C_v = float(interp((d_eval, pos_eval)))
...
@lru_cache(maxsize=8)
def _cv_interpolator(valve_type):
    data = _VALVE_DATA[valve_type]
    return RegularGridInterpolator((data['D'], data['Pos']), data['CV'], method='linear', bounds_error=False, fill_value=None)
